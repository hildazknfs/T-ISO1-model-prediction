...
@st.cache_data
def load_data():
    df = pd.read_csv(r"D:\HildasCollegeStuffs\PROPOSAL\Model\cleaned_dataset.csv")
    df["t (s)"] = df["t (s)"].astype(int)
    return df

@st.cache_data
def load_model():
    return joblib.load(r"D:\HildasCollegeStuffs\PROPOSAL\Model\decision_tree_model.pkl")
